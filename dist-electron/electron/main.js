...
// ── Drive: scan files
electron_1.ipcMain.handle('drive:scan', async (_e, query) => {
    ...
    const result = await apiGet(`/drive/v3/files?q=${encodeURIComponent(q)}&pageSize=200&fields=files(id,name,mimeType,webViewLink,modifiedTime)`, token);
    return result.files || [];
});
